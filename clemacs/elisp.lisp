...
(cl:defmacro %define-cxxr (name letters)
  (unless (and (cl:stringp letters)
               ...
               (cl:every (lambda (ch) (or (cl:char= ch #\a) (cl:char= ch #\d)))
                         letters))
    (cl:error "Bad cXXr spec: ~S ~S" name letters))
  (let ((form 'x))
    (loop for ch across (reverse letters) do
      (setf form (list (if (cl:char= ch #\a) 'car 'cdr) form)))
    `(cl:defun ,name (x)
    ...
    `(cl:defun (setf ,name) (new x)
       (let ((,cell x))
         ,@(loop for ch across prefix collect
             `(setf ,cell (,(if (cl:char= ch #\a) 'car 'cdr) ,cell)))
         (setf (,(if (cl:char= last #\a) 'car 'cdr) ,cell) new)
         new))))
...
Upstream `lisp/window.el` defines `display-buffer`/`pop-to-buffer` and friends
with deep window-system expectations.  During clemacs TTY bring-up we keep a
single-window model, so prefer the small compat implementations from
...
(cl:defun %maybe-install-post-load-shims (path)
  ...
  (let* ((p (and path (pathname path)))
         ...
         (type (and p (pathname-type p))))
    (when (and (stringp name) (stringp type)
               (string= (string-downcase name) "window")
               (string= (string-downcase type) "el"))
      (%install-window-display-shims)))
  nil)
