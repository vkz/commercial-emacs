...
(cl:defun next-single-property-change (pos prop &optional object limit)
  ...
  (let* ((obj (or object (current-buffer)))
    ...
    (when (and lim (>= p lim))
      (return-from next-single-property-change lim))
    (when (>= p scan-end)
...
(cl:defun next-property-change (pos &optional object limit)
  "Bring-up subset of ELisp `next-property-change'."
  (let* ((obj (or object (current-buffer)))
         (p (%text-props--normalize-pos pos obj "NEXT-PROPERTY-CHANGE"))
         (lim (and limit (%text-props--normalize-pos limit obj "NEXT-PROPERTY-CHANGE")))
         (max (%text-props--max-pos obj))
         (scan-end (or lim max)))
    (when (and lim (= p lim))
      (return-from next-property-change p))
    (when (and lim (>= p lim))
      (return-from next-property-change lim))
    (when (>= p scan-end)
      (return-from next-property-change nil))
    (let ((initial (text-properties-at p obj)))
      (loop for i from (1+ p) below scan-end do
        (unless (%plist-equal-as-set (text-properties-at i obj) initial)
          (return-from next-property-change i)))
      (if lim lim nil))))
...
(cl:defun previous-single-property-change (pos prop &optional object limit)
  ...
  (let* ((obj (or object (current-buffer)))
    ...
    (when (and lim (<= p lim))
      (return-from previous-single-property-change lim))
    (when (<= p min)
...
(cl:defun previous-property-change (pos &optional object limit)
  "Bring-up subset of ELisp `previous-property-change'."
  (let* ((obj (or object (current-buffer)))
         (p (%text-props--normalize-pos pos obj "PREVIOUS-PROPERTY-CHANGE"))
         (lim (and limit (%text-props--normalize-pos limit obj "PREVIOUS-PROPERTY-CHANGE")))
         (min (%text-props--min-pos obj)))
    (when (and lim (= p lim))
      (return-from previous-property-change p))
    (when (and lim (<= p lim))
      (return-from previous-property-change lim))
    (when (<= p min)
      (return-from previous-property-change (and lim lim)))
    (let* ((ref (1- p))
           (scan-start (or lim min))
           (initial (text-properties-at ref obj)))
      (loop for i from (1- ref) downto scan-start do
        (unless (%plist-equal-as-set (text-properties-at i obj) initial)
          (return-from previous-property-change (1+ i))))
      (if lim lim nil))))
