...
- DONE (2026-01-13): More `startup-check` undefined-var warnings (`PAGE-DELIMITER`, `PROCESS-ENVIRONMENT`) fixed by defvar-ing the underlying globals early in `clemacs/elisp-compat/00-core.lisp` (with microtests).
- DONE (2026-01-13): Triage top `sbcl.latest.log` undefined vars: defvar core minibuffer/completion globals (`MINIBUFFER-DEFAULT`, `COMPLETION-AUTO-SELECT`, `COMMAND-LINE-ARGS-LEFT`, etc.) early in `clemacs/elisp-compat/00-core.lisp`, and add a `:emacs :match` microtest to lock `boundp` parity.
...
- DONE (2026-01-13): Fix `ucs-normalize` load in `tty-editor`: make `get-char-code-property` return CCC=0 by default, and clamp out-of-range char-table indices so `regexp-opt-charset` can handle non-BMP codepoints without aborting startup.
- DONE (2026-01-13): Raised `lisp/minibuffer.el` cap 40 -> 50 in `startup.tty-editor`.
- DONE (2026-01-13): Finish env stack: uncap `lisp/env.el` in `clemacs/contract/startup.check.files`, implement `setenv`/`setenv-internal` (and `getenv-internal` semantics) so `process-environment` mutations are stable, add microtests, and promote the matching `test/lisp/env-tests.el` cluster into `clemacs/contract/ert-upstream.tests`.
- DONE (2026-01-13): Fix Emacs regexp numbered groups: support `\\(?N:...)`/`\\(?:...)` translation, map match-data for explicitly-numbered capture groups, and update buffer regex entrypoints (`looking-at`, `re-search-*`, `looking-back`) for the new cached-scanner tuple (restores `isearch`/`clemacs:test:tty`).
...
- DONE (2026-01-12): Complete `pdump.tty-editor` coverage (105/105) by adding the remaining tail (`image.el`, `fontset.el`, `vc-hooks.el`, `paren.el`, `eldoc.el`, etc) and raising many prior 10-caps to 50; fix the first surfaced loader blockers (early keymaps `key-translation-map`/`input-decode-map`, obarray-backed `intern`, `completion-table-dynamic`, and temp-file/auto-save globals); keep `ucs-normalize.el` capped conservatively for now.
