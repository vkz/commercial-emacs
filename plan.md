...
- 2026-01-07: split `clemacs/elisp-compat.lisp` into `clemacs/elisp-compat/*.lisp` modules; advanced `startup.check.files` (`lisp/loaddefs.el` to 400, `lisp/emacs-lisp/seq.el` to 100, `lisp/disp-table.el` to 60, started `lisp/ls-lisp.el` at 1) with incremental fixes (`standard-display-table`, `cl-defgeneric` shim for `seq.el`).
