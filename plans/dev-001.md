# Dev Notes Archive 001 (Step 0/1 iteration)

Date: 2026-01-02

This file snapshots the dev notes captured during the first big prep
iteration. For current notes, see `plans/dev.md`.

Status summary (Iteration 001)
- DONE: Tight trim safety loop via `mise run trim:check` + `mise run verify`.
- DONE: TTY-only build on macOS is repeatable and incremental.
- DONE: Native-comp removed/disabled (bootstrapping-safe stubs kept).
- DONE: Dynamic modules removed (and their test resources removed).
- DONE: GUI backends removed (dirs + `src/` backends); `src/xwidget.h` kept for stubs.
- DONE: GUI-only Elisp integration removed (term/*win, x-dnd/pgtk-dnd + test).
- DONE: Manuals are opt-in via `mise run docs:*` (no implicit `info/dir` regen).

--- Snapshot begins (original contents of `plans/dev.md`) ---
