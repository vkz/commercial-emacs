# Plan Archive 001 (Step 0/1: TTY-only trim + mise harness)

Date: 2026-01-02

This file snapshots the project plan *as it stood* after completing the first
major prep iteration (Step 0/1 trimming + build/test harness work).

Status summary (Iteration 001)
- DONE: TTY-only build direction established (no NS/X11/PGTK runtime).
- DONE: Native compilation (libgccjit / .eln) removed/disabled.
- DONE: Dynamic modules removed.
- DONE: GUI backends removed:
  - Directories: `nextstep/`, `lwlib/`, `oldXMenu/`
  - `src/` GUI backends (NS/X11/PGTK); keep `src/xwidget.h` for TTY stubs.
- DONE: GUI-only Elisp integration removed:
...
- DONE: Repo regen surface reduced:
  - `admin/unidata/` removed; Unicode test vectors moved under `test/data/`.
  - Non-build-critical `admin/*` tooling pruned.
- DONE: mise tasks added for a tight, repeatable loop:
  - `bootstrap`, `configure:tty`, `build`, `run`, `verify`, `test:*`,
    `trim:check`, `doctor`, `clean`, `clobber`, `docs:*`.
- DONE: Acceptance criteria met on macOS:
  - `mise run verify -- --level check` builds `emacs`/`emacsclient` and passes
    the default `make -C build/.../test check` suite (TTY-only).

Key commits (not exhaustive)
- `4b2fb0ba6e7` trim: remove legacy ports and native compilation
- `d258785163d` build: add mise tasks for tty build and tests
- `f1833f21ea2` trim: remove dynamic modules
- `4c1f5a5e6ba` trim: prune admin tooling
- `cead33fa6e8` trim: remove src GUI backend sources
- `6b736ba3292` trim: drop GUI-only elisp integration
