...
- If a post-load shim doesn't "stick" in the command loop, ensure you're overriding the ELisp function cell via `fset` (not only CL `fdefinition`), since ELisp `funcall`/`apply` resolve symbols through `symbol-function` and `*elisp-function-cells*`.
...
- For PTY-driven TTY debugging, set `CLEMACS_TTY_TRACE_LOG=build/clemacs/tmp/tty-trace.log` to log `(keys cmd buffer point last-command this-command)` for each `tty-main` iteration.
- For PTY-driven TTY failure triage, set `CLEMACS_TTY_DEBUG_ERRORS=1` and `CLEMACS_TTY_ERROR_LOG=$(pwd)/build/clemacs/tmp/tty-error.log` (absolute path) to capture `(signal ...) keys=... cmd=...` for the first error.
- If `window.el` is loaded and TTY commands hang inside `window-size`/`window-pixel-width`, suspect a recursion loop between upstream `lisp/window.el`’s `window-size` and clemacs’ `window-pixel-width`, and restore the clemacs window shims after `window.el` loads (see `clemacs/elisp.lisp` `%maybe-install-post-load-shims`).
...
- If an upstream ERT suite passes in `clemacs:test:ert-upstream-one` but fails in `clemacs:test:ert-upstream`, check for stale function-cell state: `fmakunbound` must clear `*elisp-function-cells*` so `(fboundp sym)` is actually nil and upstream reset logic (e.g. cl-generic) takes effect.
- If `interactive-form` blows the stack during advice bring-up, ensure it does not treat CL function objects’ debug names as ELisp symbols (only resolve `symbol-function` when the input is a symbol).
- If `build/clemacs/reports/progress.md` shows stale tty-editor bindings, remember it probes `build/clemacs/bin/emacs` (saved executable); after compat changes, rebuild it with `mise run clemacs:emacs:build` before trusting the meter.
